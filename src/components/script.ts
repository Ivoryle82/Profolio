async function main(): Promise<void> {
    ...
    if (!code) {
        ...
        let verifier: string | null = localStorage.getItem("verifier");
        if (!verifier) {
            // If verifier is not found in localStorage, generate a new one
            verifier = generateCodeVerifier(128);
            localStorage.setItem("verifier", verifier);
            console.log("Generated new code verifier:", verifier);
        }

        const accessToken: string = await getAccessToken(clientId, code, verifier);
...
export async function getAccessToken(clientId: string, code: string, verifier: string): Promise<string> {
     ...
     const checkverifier = localStorage.getItem("verifier");

     // Log the code verifier to the console for debugging
     console.log("Code Verifier:", checkverifier);
    const tokenParams: URLSearchParams = new URLSearchParams();
